    ...
    con.query("SELECT id FROM account WHERE id=" + id, function (err, result, fields) {
        ...
        if (message.text == start&&result.length===0) {
            ...
            con.query("insert into `account` SET ?", user, function (error, results) {
                ...
                console.log(error)
                ctx.reply('welcome ' + ctx.from.first_name + ' to Bch shop.\n\n🔹Own shops\n🔹Collect income\n🔹Exchange income for 💰\n🔹Earn real Money in BCH', Markup
                    .keyboard([
        ...
        if (results[0].depoaddre== null) {
            client.getAccount(btc, function (err, account) {
                account.createAddress(null, function (err, address) {
                    ...
                    ctx.replyWithHTML('<b>⚡Top up balance</b>\n\nYou can buy game currency for purchasing shops and (💰) using BCH. To top up your balance, simply send<b> any amount</b> of BCH to this address\n\n<code>' + adress + '</code>\n\nThe BCH will be automatically credited to the balance at the rate of\n' + '<b>0.1 BCH = 100,000 💰</b>\n' + '<b>0.01 BCH = 10,000 💰</b>\n' + '<b>0.001 BCH = 1,000 💰</b>\n' + '<b>0.0001 BCH = 100 💰 </b>etc.')
                    var ide =ctx.from.id
...
greeterScene.on('message', (ctx) => {
    if (isLegacyAddress(ctx.message.text)) {
        var ide = ctx.from.id
    ...
    } else if (isBitpayAddress(ctx.message.text)) {
        var ide = ctx.from.id
        var sqli = "update `account` set `withdrawadd` = '" + ctx.message.text + "' where `id` = '" + ide + "'";
        con.query(sqli)
        ctx.replyWithHTML('<b>withdraw address updated</b>', Markup
            ...
            .extra())
        ctx.scene.leave()
    }else if (isCashAddress(ctx.message.text)) {
        var ide = ctx.from.id
        var sqli = "update `account` set `withdrawadd` = '" + ctx.message.text + "' where `id` = '" + ide + "'";
        con.query(sqli)
        ctx.replyWithHTML('<b>withdraw address updated</b>', Markup
        ...
        ctx.reply('invalid BCH address', Markup
            .keyboard([
                ['🛑cancel']
            ])
